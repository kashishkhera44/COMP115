...
def dim_eight_pixels_image():
    ...
    for row in range(im.height):
        for col in range(im.width):
            ...
            # Dim the pixel by halving its RGB values
            r = int(r / 2)
            g = int(g / 2)
            b = int(b / 2)
            
            # Set the dimmed pixel in the new image
...
def decode_image():
    ...
    for row in range(im.height):
        for col in range(im.width):
            ...
            # Check if the red channel is odd
            if r % 2 == 1:
                # If odd, set the pixel in the secret image to black
                im_secret.putpixel((col, row), (0, 0, 0))
            else:
                # If even, set the pixel in the secret image to red
