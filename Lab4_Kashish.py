...
def convert_american_dollars(american_dollars):
    # pass
    canadian_dollar = 1* 1.35
    return round(american_dollars,2)

# convert_american_dollars(100) 
...
def alarm_time(current_time_string, waiting_time_string):
    pass 
    hours = current_time_string + waiting_time_string
    timeofday = hours % 24
...
def find_max(nums):
    ...
    return(max_value)
assert find_max([2,3,5]) ==5
assert find_max([5,18,9]) == 18
...
assert evens([1,4,7,16]) == [4,16]
assert evens([14,4,17,6]) == [14,4,6]
...
assert sum_of_squares([2,8,14]) == 264
...
def squares(nums):
    ...
    for num in nums:
        squared_value = num ** 2
        squares_lists.append(squared_value)
    return (squares_lists)
assert squares([1, 2, 3, 4]) == [1, 4, 9, 16]
assert squares([1, 4, 3, 6]) == [1, 16, 9, 36]
# Write and pass your units tests (5 marks)
