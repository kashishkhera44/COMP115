...
def remove_duplicates(s):
    """
    Removes duplicate characters in a string while maintaining the original order of characters.

    Parameters:
    - s (str): The input string containing duplicate characters.

    Returns:
    - str: The string with duplicate characters removed, maintaining the original order.
    """
    seen = set()
    result = ''
    for char in s:
...
def find_index(s, t):
    """
    Returns lowest index of a character found in a string

    Parameters:
    - s (str) : The input string to found the character
    - t( str) : The character in the string that we need to find in the string 

    Returns :
    - (int) : The lowest index of characters in that string. Return -1 if the character is not in the string.
...
def project_completion_day(day, days_to_completion):
    """
    Returns the project completion day given the current day and estimated time of days to completion.
    ...
    for _ in range(days_to_completion):
        current_day_index = (current_day_index + 1) % 7
    return days_week[current_day_index]
# Your unit tests
